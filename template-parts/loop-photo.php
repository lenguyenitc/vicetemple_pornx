...
{ post_class('thumb-block full-width album'); }else{ post_class('thumb-block album'); } ?>>
	<a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>">
        ...
        <header class="entry-header photos-entry-header">
            ...
            <p class="rating-bar">
				<?php if(xbox_get_field_value( 'my-theme-options', 'enable_view' ) == 'on') : ?><span class="views">
					<?=(get_post_meta($post->ID, 'photo_gallery_views', true)) ? get_post_meta($post->ID, 'photo_gallery_views', true) : '0';?> <span class="viewers">views</span></span><?php endif; ?>
                <span class="author-photos"><?php echo get_the_author($post->ID)?></span>
            </p>
        </header>
	</a>
</article>