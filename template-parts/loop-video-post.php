<article id="post-<?php $post->ID; ?>" <?php if(xbox_get_field_value( 'my-theme-options', 'mob-number_videos_per_row' ) == '1') { post_class('thumb-block full-width'); }else{ post_class('thumb-block'); } ?>>
	<?php
    ...
    <a href="<?php echo $permalink; ?>" title="<?php the_title(); ?>" data-toggle="<?php echo $data_toggle;?>" data-target="<?php echo $data_target;?>">
		...
		<?php if( $trailer_url != '' /*&& !wp_is_mobile()*/ && $trailer_url !== false && $trailer_url !== 'false' && $trailer_url !== 'http://false' && $trailer_url !== 'https://false') : ?>
            ...
            <div class="post-thumbnail video-with-trailer">
				...
				<?php endif; ?>
				<?php if(has_term('watchlater'.$user->ID, 'playlists', $post->ID)):?>
                    <span data-add="add" data-user="<?php echo $user->ID;?>" class="watchLaterIcon<?=$favoriteLogin?>" data-post="<?php $post->ID; ?>" style=" display: none"><i class="fa fa-check" style="font-size: 24px"></i></span>
				<?php else: ?>
                    <span data-add="" data-user="<?php echo $user->ID;?>" class="watchLaterIcon<?=$favoriteLogin?>" data-post="<?php $post->ID; ?>" style=" display: none"><i class="fa fa-plus" style="font-size: 24px"></i></span>
				<?php endif;?>
            </div>
		<?php else : ?>
            <!-- Thumbnail -->
